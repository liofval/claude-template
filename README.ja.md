...
rules と examples に基づいてコードが自動生成されます。
...
### 3. Layer Rules をカスタマイズ

> [!IMPORTANT]
> Layer Rules（`.claude/rules/*.md`）は **TypeScript / Next.js / Prisma** をデフォルトとして記述されています。**プロジェクトの技術スタックに合わせて必ず書き換えてください。** Core Rules（`core/`）は言語非依存のためそのまま使えます。

Layer Rules は2段階に分かれています。**まず必須の4つだけカスタマイズすればOK** — 残りは必要になったら追加してください。

| 分類 | ファイル | カスタマイズのタイミング |
|---|---|---|
| **必須** | `api.md`, `backend.md`, `db.md`, `config.md` | 最初にプロジェクトの技術スタックに書き換える |
| **任意** | `frontend.md`, `hooks.md`, `testing.md`, `types.md`, `cicd.md`, `docs.md` | 該当レイヤーを使うときに追加 |

各ルールは **Do/Don't + Example** 形式で記述されています：
...
### レイヤールール — 必須（最初にカスタマイズ）

| ルールファイル | 対象パス | 主なポイント |
|---|---|---|
| `api.md` | `src/app/api/**`, `src/pages/api/**` | RESTful設計、Zodバリデーション、統一エラーフォーマット |
| `backend.md` | `src/lib/**`, `src/services/**`, `src/utils/**` | カスタムエラークラス、テストのコロケーション |
